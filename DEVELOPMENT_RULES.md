...
## 🛡️ 6. 유지보수 및 보존 규칙 (Preservation Rules)
### 6.1 디자인 및 로직 보존 원칙
- **명시적 요청 우선**: 사용자가 수정을 명시적으로 요청한 부분 외의 기존 디자인, 레이아웃, CSS 스타일, 비즈니스 로직은 **절대 수정하거나 삭제하지 않는다.**
- **임의 리팩토링 금지**: "가독성 개선"이나 "코드 최적화"라는 명목으로 기존에 정상 작동하는 코드를 임의로 재작성하지 않는다.
- **영향 범위 최소화**: 수정이 필요한 경우, 해당 변경 사항이 다른 페이지나 기능에 미치는 영향을 최소화하도록 코드를 작성한다.
- **패턴 유지**: 새로운 기능을 추가할 때도 기존 프로젝트에서 사용 중인 변수 명명 규칙, 클래스 구조, 이벤트 처리 방식을 그대로 계승한다.
- **예외 사항**: 버그 수정이나 보안 취약점 해결을 위해 불가피하게 로직을 건드려야 할 경우, 반드시 사용자에게 사전 고지하고 승인을 얻는다.
